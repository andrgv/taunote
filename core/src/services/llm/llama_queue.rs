...
use tokio::{
    sync::{mpsc::{self, Sender}, oneshot, Mutex},
    task,
};
use once_cell::sync::OnceCell;
use anyhow::{Error as AnyhowError, anyhow, Result};
...
pub async fn init_llama_queue() {
    ...
    task::spawn(async move {
        let client = match LlamaClient::try_new(
            ...
            Ok(c) => c,
            Err(err) => { eprintln!("Failed to start LlamaClient: {}", err); return; }
        };
        
        // process jobs from the queue as they arrive
        while let Some(CompletionJob { prompt, n_predict, responder }) = rx.recv().await {
...
pub async fn enqueue_completion(
    ...
    let job = CompletionJob { prompt, n_predict, responder: tx };
